...
class Admin extends CI_Controller {
	...
	function __construct() {
		...
		//load custom module
			...
			$this->load->model('admin_companies');
	...
	public function index()
	{
		...
		$config['base_url'] = site_url(ADMIN_DASHBOARD_PATH).'/companies/index/'.$status;

		$config['total_rows'] = $this->admin_companies->get_total_companies($status);
		...
		$this->data['result_data'] = $this->admin_companies->get_companies_details($this->uri->segment(4),$config['per_page'],$offset);

		$this->data['active_menu'] = 'member';

        $this->data['active_submenu'] = 'companies';

        $this->data['modules_name'] = 'Companies Management';

        $this->data['modules_heading'] = 'View Companies Detail';
		...
		$this->template
			...
			->title('Companies View | '. SITE_NAME)
...

	{
		...
		if(!isset($id)){redirect(ADMIN_DASHBOARD_PATH.'/companies/index/'.$status,'refresh');exit;}

		

		$this->data['profile'] = $this->admin_companies->get_company_byid($id);
		

		//check data, if it is not set then redirect to view page

		if($this->data['profile'] == false){redirect(ADMIN_DASHBOARD_PATH.'/companies/index/'.$status,'refresh');exit;}


		//Set the validation rules

		$this->form_validation->set_rules($this->admin_companies->validate_settings_edit);
...

		{
			...
			$this->admin_companies->update_record($id);

			$this->session->set_flashdata('message','The company records are update successful.');

			redirect(ADMIN_DASHBOARD_PATH.'/companies/index/'.$status,'refresh');			
        ...
        $this->data['modules_heading'] = 'Edit Company';
        ...
        $this->breadcrumb->populate(
        	...
        	array(
        		...
        		'Member Management' => '#',
        		'Companies Management' => ADMIN_DASHBOARD_PATH.'/companies/index/'.$status,
        		'Edit company' => '#'
		...
		$this->template
			...
			->title('Companies Edit | '. SITE_NAME)
...

	{
		...
		$query = $this->db->get_where('members', array('id' => $id));
...

			{
				...
				$this->session->set_flashdata('message','The company record delete successful.');
...

	{
...

		{
			...
			//echo $this->admin_companies->change_company_password();		

			$res = array(

				'status'=>'success',

				'message'=>$this->admin_companies->change_company_password()
	...
	**/
...

	{
...

		{
			...
			$this->form_validation->set_rules($this->admin_companies->validate_settings_add);
			...
			if($this->form_validation->run() === TRUE)
			{
				...
				$member_id = $this->admin_companies->insert_company($salt, $password);
				...
				if($member_id)
				{
					...
					$this->admin_companies->register_notification($member_id, $rand_password);

					$this->session->set_flashdata('message','Company added successfully');

					redirect(ADMIN_DASHBOARD_PATH.'/companies/index/1', 'refresh');
...

				{
					...
					$this->session->set_flashdata('message', 'Company add failed');
        ...
        $this->data['modules_heading'] = 'Add Company';
        ...
        $this->breadcrumb->populate(
        	...
        	array(
        		...
        		'Member Management' => '#',
        		'Companies Management' => ADMIN_DASHBOARD_PATH.'/companies/index/1',
        		'Add company' => '#'
	...
	public function member_activation_mail($user_id)
	{
		...
		$member_activation_mail_status = $this->admin_companies->reg_confirmation_email($user_id);
		...
		redirect(ADMIN_DASHBOARD_PATH.'/companies/index/0', 'refresh');
	}
