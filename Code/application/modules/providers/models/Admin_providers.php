...
class Admin_providers extends CI_Model 
{
	...
	public function get_total_providers($status)
	{		
	...
	public function get_providers_details($status,$perpage,$offset)
	{
		if($status) $status = $status; else $status = '0';
	...
	public function get_provider_byid($id)
	{			
				 $this->db->select('m.*');
	...
	public function count_provider_transaction($user_id)
	{
		$option = array('user_id'=>$user_id,'transaction_status'=>'Completed');
		$query = $this->db->get_where('transaction',$option);
		return $query->num_rows();
	}
	public function get_provider_transaction($user_id,$perpage,$offset)
	{
		$option = array('user_id'=>$user_id,'transaction_status'=>'Completed');
	...
	function change_provider_password()
	{		
		...
		$user_data = $this->get_provider_byid($user_id);
	...
	*
	**/
	public function insert_provider($salt, $password)
	{
